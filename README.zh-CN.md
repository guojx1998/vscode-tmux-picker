...
开一个终端，在它接入 tmux **之前**就弹出一个终端内菜单：输名字新建会话，或用方向键挑一个已有的（显示它是否已连、当前目录）。要点：每个终端都**透明地**跑在 tmux 里，所以正在跑的东西在断连、重载窗口、合盖休眠后都不中断。
...
VS Code 不会让远程终端进程在断连或重载窗口后存活。这是 VS Code 远程仓库里**票数最高、开了 6 年仍未做**的需求 [vscode-remote-release#3096][3096]。通行的兜底是把每个集成终端**透明地**跑在一个 **tmux** 会话里：这样重载窗口、SSH 掉线、服务器重启都不会杀掉正在跑的东西。合上笔记本，明天换一台机器重连，你的 shell、编译、REPL 还都在。
...
- bash ≥ 4.3（i18n 表用到关联数组 + nameref）
- tmux ≥ 3.3
- Linux 上的 VS Code 集成终端（Remote-SSH 或本地）。持久化的收益面向 **Remote-SSH**；本地用它依然是个会话选择器。
...
- 仅支持 bash 4.3+（macOS 自带 3.2，本地用的话 `brew install bash`）。
